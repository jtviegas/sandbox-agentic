...
reqs(){
  ...
  if [ $? -ne 0 ] ; then
    info "[mysys_tools] installing ollama"
...
run_ollama_model(){
  info "[run_ollama_model|in] ($1)"
  local MODEL_NAME="$1"
  [[ -z "$MODEL_NAME" ]] && err "[run_ollama_model] missing MODEL_NAME argument" && exit 1

  local result=0
  ollama list "$MODEL_NAME" >/dev/null 2>&1
  if [ $? -ne 0 ] ; then
    info "[run_ollama_model] model $MODEL_NAME not found locally, pulling from remote"
    ollama pull "$MODEL_NAME"
    result="$?"
    if [ ! "$result" -eq "0" ] ; then err "[run_ollama_model] failed to pull model $MODEL_NAME"; fi
  else    info "[run_ollama_model] model $MODEL_NAME found locally"
  fi
  
  ollama run "$MODEL_NAME"
  result="$?"
  if [ ! "$result" -eq "0" ] ; then err "[run_ollama_model] failed to run model $MODEL_NAME"; fi

  local msg="[run_ollama_model|out] => ${result}"
  [[ ! "$result" -eq "0" ]] && info "$msg" && exit 1
  info "$msg"
}
...
usage() {
  ...
  $(basename $0) { option }
    options:
      ...
      - get_latest_tag
      - run_ollama_model <MODEL_NAME>        checks if the specified Ollama model is available locally, pulls it if not, and runs it
EOM
  exit 1
}
